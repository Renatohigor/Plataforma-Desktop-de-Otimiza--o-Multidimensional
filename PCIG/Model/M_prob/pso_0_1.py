...
class Swarm_1:
    ...
    def __init__(self,n):
        ...
        self.population=[]
        self.domain = None
        nomeAlgo ='PSO-STANDART  08/08/2019'
    ...
    def minimum(self, f):
        ...
        for i in range(0,self.n):
            ...
            self.population[i].fit= 1 + abs(self.population[i].of) if self.population[i].of <=0 else  1/(1+self.population[i].of)
    ...
    def update_population_position_velocity(self,iter):
        ...
        for i in range(0, len(self.population)):
            self.population[i].update_speed_and_position(self.gBest,self.domain)
            self.population[i].update_pBest(self.f, iter)
        return aux

    def PSO(self, f, domain, itr,initial_velocity):
        self.f=f
        self.domain=[domain[0].v,domain[-1].v]
        self.itr=itr
        self.boot_the_population(self.domain,initial_velocity)
