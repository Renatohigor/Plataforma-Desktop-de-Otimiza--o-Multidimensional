...
class Colony_03_1:
    ...
    def moveUndivided(self,dim, x):
        ...
        for i in range(1, len(self.colony)):
            ...
            for j in range(0, i):
                ...
                beta = self.beta_min + (self.beta0 - self.beta_min) * exp(B)
                mudou = self.colony[i].update_toMove(alpha=self.alpha, position_j=ns0[j].position, beta=beta,
                                                           d=dim, f=self.f)
                if mudou:
    ...
    def main(self, f, dim, n, beta0,beta_min,  alpha, numGer, paraAbsorcao, tetha):
        ...
        while x <= self.numGer:
            ...
            self.ranking()
            self.alpha = (1/9000)**(1/x) * self.alpha
            self.moveUndivided(dim,x)
            self.evaluateNewSolutions(f)
            x+=1


        return self.colony[0].position