...
class GA(object):
    ...
    def population_create(self):
        for i in range(self.population_size):
            self.chromosome_list.append(Chromosome(self.function_problem, None,self.dominio))
    ...
    def crossover(self):
        ...
        aux1 = [0] * self.function_problem.n
        aux2 = [0] * self.function_problem.n
        for j in range(self.function_problem.n):
            aux1[j] = [0] * 64
            aux2[j] = [0] * 64
            r_pos = randint(0, 63)
            for i in range(64):
        ...
        self.chromosome_list.append(Chromosome(self.function_problem, aux1,self.dominio))
        self.chromosome_list.append(Chromosome(self.function_problem, aux2,self.dominio))

    def mutation(self):
        r_dimension = randint(0, self.function_problem.n - 1)
...
class Chromosome(object):
    ...
    def __init__(self, func, values,dominio):
        ...
        self.calc_adjusted_value(dominio)
        aux=Ponto()
        aux=Ponto.v=self.real_values

        self.func_value = self.func<<aux

    def generate_random_digits(self):
        for i in range(self.func.n):
            d = []
            for j in range(self.bits_length):
                d.append(1 if random() > 0.5 else 0)
            self.bits_values.append(d)

    def calc_adjusted_value(self,dominio):

        for i in range(self.func.n):

            limit_a, limit_b = dominio[0],dominio[1]
